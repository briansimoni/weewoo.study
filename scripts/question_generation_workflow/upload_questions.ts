...
async function main() {
  ...
  try {
    ...
    console.log(
      `📚 Found ${questionsFile.questions.length} questions to upload`,
    );
      ...
      for (let j = 0; j < batch.length; j++) {
        ...
        try {
          ...
          if (errorMessage.includes("Question already exists")) {
            if (config.skip_duplicates) {
              ...
              console.log(
                `⏭️  Question ${
                  questionIndex + 1
                }: Skipped (duplicate) - "${questionPreview}"`,
              );
              continue;
            } else {
          ...
          for (
            ...
            try {
              console.log(
                `🔄 Retry ${retryCount}/${config.max_retries} for question ${
                  questionIndex + 1
                }`,
              );
              ...
              console.log(
                `✅ Question ${
                  questionIndex + 1
                }: "${questionPreview}" (retry ${retryCount})`,
              );
              ...
              if (retryCount === config.max_retries) {
                ...
                result.errors.push({
                  ...
                  question_preview: questionPreview,
                  error:
                    `Failed after ${config.max_retries} retries: ${retryErrorMessage}`,
                });
                console.error(
                  `❌ Question ${
                    questionIndex + 1
                  }: Failed after retries - "${questionPreview}"`,
                );
              }
            }
          }
    ...
    // Save upload results
    const outputPath = config.db_ready_questions_file.replace(
      ".json",
    ...
    console.log("\n📊 Upload Results:");
    console.log(
      `✅ Successfully uploaded: ${result.successful_uploads} questions`,
    );
    console.log(
      `⏭️  Skipped duplicates: ${result.skipped_duplicates} questions`,
    );
    console.log(`❌ Failed uploads: ${result.failed_uploads} questions`);
    console.log(
      `📈 Success rate: ${
        ((result.successful_uploads / result.total_questions) * 100).toFixed(2)
      }%`,
    );
