...
const WEBHOOK_URL = "https://test.weewoo.study.com";
  ...
  try {
    ...
    if (existingWebhooks.result && existingWebhooks.result.length > 0) {
      ...
      for (const webhook of existingWebhooks.result) {
        console.log(`- ID: ${webhook.id}, URL: ${webhook.url}, Types: ${webhook.types.join(", ")}`);
        
        // Delete if it has the same URL (to avoid duplicates)
        if (webhook.url === WEBHOOK_URL) {
          console.log(`Deleting existing webhook with ID ${webhook.id} that points to the same URL...`);
    ...
    const result = await printfulClient.createWebhook(WEBHOOK_URL, [EVENT_TYPE]);
