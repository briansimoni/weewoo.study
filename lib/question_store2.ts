...
export class QuestionStore2 {
  ...
  static async make(kv?: Deno.Kv, scope: "emt" | "advanced" | "medic" = "emt") {
    ...
    return new QuestionStore2(kv, scope);
  }

  async add(
    question: Omit<Question2, "id" | "created_at" | "updated_at" | "scope">,
  ): Promise<Question2> {
  ...
  async delete(questionId: string): Promise<void> {
    const questionResult = await this.kv.get<Question2>([
      this.scope,
      "question_content",
      questionId,
    ]);
   ...
   */
  updateQuestion(_question: Question2): Promise<Question2> {
    return Promise.reject(new Error("Not implemented"));
  }

  async getQuestionById(questionId: string): Promise<Question2> {
    const questionResult = await this.kv.get<Question2>([
      this.scope,
      "question_content",
      questionId,
   ...
   */
  async listQuestions(category?: string): Promise<Question2[]> {
    ...
    if (category) {
      ...
      const cursor = this.kv.list<Question2>({
  ...
  async getRandom(): Promise<Question2> {
    ...
    const question = (await this.kv.get<Question2>([
      this.scope,
      "question_content",
  ...
  async getRandomByCategory(category: string): Promise<Question2> {
    const { value: count } = await this.kv.get<number>([
      this.scope,
      "category_question_count",
      category,
